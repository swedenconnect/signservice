...
class SignServiceRSASignerTest {
  ...
  static void init() throws Exception {
    ...
    signature = Base64.decode(
      "PcwB/C4xz9x3ce+nrv4n8F7daWF8IxbrGFmY8dDd1XmRwhQzt1mQjfdMeFtmD2uZvxbFtyIe/cOJZbLkez36ZMj5iuX4F8Dw0pLz"
    ...
    byte[] certBytes = Base64.decode(
      "MIIKzzCCCTegAwIBAgIQAy9wRRYfzQOP8bP9fID9UzANBgkqhkiG9w0BAQsFADCBhzELMAkGA1UEBhMCU0UxFjAUBgN"
  ...
  void errorTests() throws Exception {
    ...
    specificErrorTest(tbsData, null, rsaSha256, NullPointerException.class);
    ...
    specificErrorTest(tbsData, TestCredentials.privateRSAKey, TestAlgorithms.rsaPssSha256, SignatureException.class);
    log.info("Error test ecdsa key:");
    specificErrorTest(tbsData, TestCredentials.privateECKey, rsaSha256, SignatureException.class);
    log.info("Error test null data:");
    specificErrorTest(null, TestCredentials.privateRSAKey, rsaSha256, SignatureException.class);
  }
  ...
  void specificErrorTest(byte[] tbsData, PrivateKey privateKey, SignatureAlgorithm signatureAlgorithm,
    ...
    Exception signatureException = assertThrows(exceptionClass, () -> {
  ...
  void signRsaWithAlgo(SignatureAlgorithm signatureAlgorithm) throws Exception {
    ...
    boolean verifiedSignedData = PKCS1V15Padding.verifyMessageDigest(decryptedSignature, messageDigest,
      signatureAlgorithm.getMessageDigestAlgorithm());
