...
public class EcdsaSigValue implements ASN1Encodable {
  ...
  @Getter @Setter private static int[] supportedKeyLengths = new int[] { 160, 224, 256, 384, 521 };

  /** @return the R component of the ECDSA Signature */
  @Getter private final BigInteger r;
  /** @return the S component of the ECDSA Signature */
  @Getter private final BigInteger s;
   ...
   * Creates an instance of ECDSA signature value
   *
   * @param obj signature value object as {@link EcdsaSigValue}, {@link ASN1Sequence} or {@link ASN1InputStream}
   * @return ECDSA signature value
   * @throws IOException invalid input
   */
   ...
   * Creates an instance of ECDSA signature value
   *
   * @param a1s ASN1 sequence of ECDSA signature value
   * @return ECDSA signature value
   * @throws IOException invalid input
   */
  ...
  private EcdsaSigValue(ASN1Sequence obj) throws IOException {
    try {
      ...
      throw new IOException("Error creating ECDSA signature value from provided ASN1 sequence");
    }
  }
  ...
  private static BigInteger[] getRSFromConcatenatedBytes(byte[] concatenatedRS) throws IOException {
    try {
      ...
      throw new IOException("Unable to parse R S integers from concatenated bytes", ex);
    }
  }
  ...
  /**
   * Gets the bytes to be carried in an OCTET STRING to form the CMS
   * signature value
   *
  ...
  /**
   * Return the key length that represents the signature value block size. For EC 256 bit key the block size is 256.
   * For EC 521 bit key the block size is 528 (the bits of the full byte count that can contain the key).
   *
   * <p>Calculation of the block size take into account that the actual integer byte representation may be different than
   * the block size/8. This is the case when the bytes representation of the integer is padded to avoid negative representation as
   * well as the situation when the byte count is shorter since the actual integer is smaller than the allowed max size</p>
   *
   * <p>For this reason this implementation lists allowed key sizes supported by this implementation and present integers must fit these key
   * sizes. A match is when the size without padding is an exact match or when the sum size difference between both integer byte count and
   * 2 x the block size byte count is not grater than 4 bytes.</p>
   *
  ...
  /**
   * Return the actual number of bytes representing the value of an integer not counting any padding bytes
   * Since the bytes are derived form a BigInteger, the maximum number of present padding bytes is 1
  ...
  private int getDataLength(BigInteger integer) {
    ...
    return integerBytes[0] == 0x00
      ? integerBytes.length - 1
      : integerBytes.length;
  }
