...
public class EcdsaSigValue implements ASN1Encodable {
  ...
  /**
   * Return the key length that represents the signature value block size. For EC 256 bit key the block size is 256. For
   * EC 521 bit key the block size is 528 (the bits of the full byte count that can contain the key).
   *
   * <p>
   * Calculation of the block size take into account that the actual integer byte representation may be different than
   * the block size/8. This is the case when the bytes representation of the integer is padded to avoid negative
   * representation as well as the situation when the byte count is shorter since the actual integer is smaller than the
   * allowed max size
   * </p>
   *
   * <p>
   * For this reason this implementation lists allowed key sizes supported by this implementation and present integers
   * must fit these key sizes. A match is when the size without padding is an exact match or when the sum size
   * difference between both integer byte count and 2 x the block size byte count is not grater than 4 bytes.
   * </p>
   *
  ...
  /**
   * Return the actual number of bytes representing the value of an integer not counting any padding bytes Since the
   * bytes are derived form a BigInteger, the maximum number of present padding bytes is 1
   *
